            ...
            <div class="tab-content pt-2" id="myTabjustifiedContent">
              <div class="tab-pane fade show active" id="tsd-justified" role="tabpanel" aria-labelledby="tsd-tab">
                <table class="table table-borderless datatable">
                  ...
                  <tbody>
                    <?php
                    $sql = "SELECT * from report rep left join user us on us.user_id = rep.user_id left join unit_details ud on ud.unit_id = us.unit_id where rep.status = 1 && us.unit_id = '" . $unit_id . "'";
                    ...
                    ?>
                        <tr>
                          ...
                          <td><?= sanitizeQuillContent($row['unit_name']) ?></td>
              ...
              <div class="tab-pane fade" id="fad-justified" role="tabpanel" aria-labelledby="fad-tab">
                <table class="table table-borderless datatable">
                  ...
                  <tbody>
                    <?php
                    $sql = "SELECT * from report rep left join user us on us.user_id = rep.user_id left join unit_details ud on ud.unit_id = us.unit_id where rep.status = 1 && revision_count >0 && us.unit_id = '" . $unit_id . "'|| rep.status = 2 && revision_count >0 && us.unit_id = '" . $unit_id . "'";
                    $result = mysqli_query($conn, $sql);
                    if (mysqli_num_rows($result) > 0) {
                      while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                        <tr>
                          ...
                          <td><?= $row['date_conducted'] ?></td>
                          <td><?php
                              if ($row['status'] == 1 && $row['revision_count'] > 0) {
                                echo "<span class='badge bg-danger'>For Revision <br>by Unit Head</span>";
                              }
                              if ($row['status'] == 2 && $row['revision_count'] > 0) {
              ...
              <div class="tab-pane fade" id="ord-justified" role="tabpanel" aria-labelledby="ord-tab">
                <table class="table table-borderless datatable">
                  ...
                  <tbody>
                    <?php
                    $sql = "SELECT * from report rep left join user us on us.user_id = rep.user_id left join unit_details ud on ud.unit_id = us.unit_id where rep.status = 3 && us.unit_id = '" . $unit_id . "'|| rep.status = 3 && rep.shared=1 || rep.status = 3 && rep.shared = 2 && rep.shared_unit = '" . $unit_id . "'";
                    $result = mysqli_query($conn, $sql);
                    if (mysqli_num_rows($result) > 0) {
                      while ($row = mysqli_fetch_assoc($result)) {
                    ?>
